...
# 先复制 setup.py 和 simpler_env（SimplerEnv 的核心）
COPY setup.py /workspace/
COPY simpler_env /workspace/simpler_env
...
# 安装 MinkowskiEngine（需要 openblas 和 PyTorch）
# 使用系统 openblas（通过 apt 安装的 libopenblas-dev）
...
RUN cd GSNet/pointnet2 && python setup.py install

# 安装 knn
RUN cd GSNet/knn && python setup.py install
