...
def get_uvd(image, depth, task_description):
    ...
    cv2.imwrite(f"results/cv_results/{ts_str} {task_description}.png", image_cv)

    # cv2.imshow(task_description, image_cv)
    # cv2.waitKey(1)
    
    if "put carrot on plate" in task_description:
        pick_sub_task = "Grasp the carrot"
        place_sub_task = task_description
    elif "put eggplant into yellow basket" in task_description:
        pick_sub_task = "Grasp the eggplant"
        place_sub_task = task_description
    elif "stack the green block on the yellow block" in task_description:
        pick_sub_task = "Grasp the green block"
        place_sub_task = task_description
    elif "put the spoon on the towel" in task_description:
        pick_sub_task = "Grasp the spoon"
        place_sub_task = task_description
    else:
        pick_sub_task = "Select grasp point of: " + task_description + "."
        place_sub_task = "Select place point of: " + task_description + "."

    (u, v), points = get_waypoints(image, pick_sub_task, "pick")
    ...
    (u, v), points = get_waypoints(image, place_sub_task, "place")
    ...
    cv2.imwrite(f"results/cv_results/{ts_str} {task_description} result.png", image_cv)
...
def fsd_execution(images, env, obs, obs_camera_name, task_description, additional_env_build_kwargs, env_reset_options, 
    ...
    planned = False
    for pick_goal_backoff in range(2):
        print("pick_goal_backoff", pick_goal_backoff)
        ...
        for idx, (_, pick_grasp) in enumerate(sorted_gs):
            try:
                ...
                print("adjusted goal", pick_goal_T)
                ...
                for try_from_qpos in [
                    ...
                    try:
                        pick_point = ik.ik(pick_goal_T[:3, 3], pick_goal_T[:3, :3], joints=ik.robot_to_arm_joints(try_from_qpos))
                ...
                if pick_point is not None:
                    ...
                    planner.pb_ompl_interface.fix_joints_value = {'left_finger': 0.037, 'right_finger': 0.037}
                    assert planner.pb_ompl_interface.is_state_valid(pick_point[:6])
                assert pick_point is not None # TODO Fails
                print("Pick Path IK Solved", pick_point)
                ...
                for _ in range(50):
                    for try_from_qpos in [
                        ...
                        try:
                            place_point = ik.ik(place_goal_T[:3, 3], place_goal_T[:3, :3], joints=ik.robot_to_arm_joints(try_from_qpos))
                    ...
                    if place_point is not None:
                        planner = Planner(cfg, planner='AITstar', fix_joints=['left_finger', 'right_finger'])
                        planner.pb_ompl_interface.fix_joints_value = {'left_finger': 0.037, 'right_finger': 0.037}
                        if planner.pb_ompl_interface.is_state_valid(place_point[:6]):
                            break
                        else:
                            print("Maybe Collision, adjust goal")
                    else:
                        print("Failed IK, adjust goal")
                    ...
                    print("adjusted goal", place_goal_T)
                ...
                for _ in range(2):
                    planner = Planner(cfg, planner='AITstar', fix_joints=['left_finger', 'right_finger'])
                    res, grasp_path = planner.plan(init_qpos, pick_point[:6], interpolate_num=100, fix_joints_value={'left_finger': 0.037, 'right_finger': 0.037}, 
                                                #    save_pc="pcd.pkl"
                                                )
                    if res:
                        break
                ...
                print('Pick Path Completed')

                for _ in range(2):
                    planner = Planner(cfg, planner='AITstar', fix_joints=['left_finger', 'right_finger'])
                    res, mid_path1 = planner.plan(pick_point[:6], mid1_point[:6], interpolate_num=100, fix_joints_value={'left_finger': 0.037, 'right_finger': 0.037})
                    if res:
                        break
