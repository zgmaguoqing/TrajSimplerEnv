...
def run_maniskill2_eval_single_episode(
    ...
    images = []
    for _ in range(3):
        images, env, obs, done, info = robopoint_execution(images, env, obs, obs_camera_name, task_description, additional_env_build_kwargs, env_reset_options)
        if done:
            break
...
def maniskill2_evaluator_robopoint(model, args):
...
from waypoint_robopoint import get_waypoints
import time
...
def robopoint_execution(images, env, obs, obs_camera_name, task_description, additional_env_build_kwargs, env_reset_options):
    image = get_image_from_maniskill2_obs_dict(env, obs, camera_name=obs_camera_name)
    images.append(image)
    ...
    gg = Grasp(translation=grasp_point2arm, rotation_matrix=pr.active_matrix_from_angle(1, math.pi/2) @ pr.active_matrix_from_angle(0, math.pi/2))
    gg_goal = Grasp(translation=place_point2arm, rotation_matrix=pr.active_matrix_from_angle(1, math.pi/2) @ pr.active_matrix_from_angle(0, math.pi/2))
    ...
    trans_2 = gg.translation - delta_trans @ rot # np.einsum('ab,b->a', rot, delta_trans)
    rot_2 = delta_rot.T @ rot # np.einsum('ba,bc->ac', delta_rot, rot) 
    ...
    delta_rot = R.from_euler('xyz', [-0.001328879239766479, -0.00443973089771006, -0.0008686511567993004], degrees=False).as_matrix()
    delta_trans = np.array([4.3374896E-03, 1.8852949E-03, 1.6353297E-01])
    trans_2 = gg_goal.translation - delta_trans @ rot
    rot_2 = delta_rot.T @ rot
